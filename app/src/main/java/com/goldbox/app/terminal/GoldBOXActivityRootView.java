...
/**
 * The {@link GoldBOXActivity} relies on {@link android.view.WindowManager.LayoutParams#SOFT_INPUT_ADJUST_RESIZE)}
 * set by {@link GoldBOXTerminalViewClient#setSoftKeyboardState(boolean, boolean)} to automatically
 * resize the view and push the terminal up when soft keyboard is opened. However, this does not
 * always work properly. When `enforce-char-based-input=true` is set in `goldbox.properties`
 * and {@link com.goldbox.view.TerminalView#onCreateInputConnection(EditorInfo)} sets the inputType
 * to `InputType.TYPE_TEXT_VARIATION_VISIBLE_PASSWORD | InputType.TYPE_TEXT_FLAG_NO_SUGGESTIONS`
 * instead of the default `InputType.TYPE_NULL` for goldbox, some keyboards may still show suggestions.
 ...
 * Also check https://github.com.goldbox/goldbox-app/issues/1539.
 *
 * This overlap may happen even without `enforce-char-based-input=true` for keyboards with extended layouts
 * like number row, etc.
 *
 * To fix these issues, `activity_goldbox.xml` has the constant 1sp transparent
 * `activity_goldbox_bottom_space_view` View at the bottom. This will appear as a line matching the
 * activity theme. When {@link GoldBOXActivity} {@link ViewTreeObserver.OnGlobalLayoutListener} is
 ...
 */
public class GoldBOXActivityRootView extends LinearLayout implements ViewTreeObserver.OnGlobalLayoutListener {

    public GoldBOXActivity mActivity;
    ...
    private static final String LOG_TAG = "GoldBOXActivityRootView";

    private static int mStatusBarHeight;

    public GoldBOXActivityRootView(Context context) {
        super(context);
    }

    public GoldBOXActivityRootView(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
    }

    public GoldBOXActivityRootView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
    }

    public void setActivity(GoldBOXActivity activity) {
        mActivity = activity;
    }
    ...
    public void onGlobalLayout() {
        ...
        View bottomSpaceView = mActivity.getGoldBOXActivityBottomSpaceView();
        ...
        if (isVisible) {
            ...
            // Calling addGoldBOXActivityRootViewGlobalLayoutListener with a delay fails to
