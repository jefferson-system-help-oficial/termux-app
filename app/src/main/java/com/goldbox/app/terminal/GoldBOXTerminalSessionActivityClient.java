...
import com.goldbox.shared.goldbox.terminal.io.BellHandler;
import com.goldbox.shared.logger.Logger;
import com.goldbox.terminal.TerminalColors;
import com.goldbox.terminal.TerminalSession;
import com.goldbox.terminal.TerminalSessionClient;
import com.goldbox.terminal.TextStyle;
...
public class GoldBOXTerminalSessionActivityClient extends GoldBOXTerminalSessionClientBase {

    private final GoldBOXActivity mActivity;
    ...
    private static final String LOG_TAG = "GoldBOXTerminalSessionActivityClient";

    public GoldBOXTerminalSessionActivityClient(GoldBOXActivity activity) {
    ...
    public void onStart() {
        ...
        if (mActivity.getGoldBOXService() != null) {
            setCurrentSession(getCurrentStoredSessionOrLast());
            goldboxSessionListNotifyUpdated();
        }
    ...
    public void onSessionFinished(@NonNull TerminalSession finishedSession) {
        GoldBOXService service = mActivity.getGoldBOXService();
        ...
        GoldBOXSession goldboxSession = service.getGoldBOXSession(index);
        if (goldboxSession != null) {
            isPluginExecutionCommandWithPendingResult = goldboxSession.getExecutionCommand().isPluginExecutionCommandWithPendingResult();
        ...
        if (mActivity.getPackageManager().hasSystemFeature(PackageManager.FEATURE_LEANBACK)) {
            ...
            if (service.getGoldBOXSessionsSize() > 1 || isPluginExecutionCommandWithPendingResult) {
    ...
    public void onBell(@NonNull TerminalSession session) {
        ...
        switch (mActivity.getProperties().getBellBehaviour()) {
            case GoldBOXPropertyConstants.IVALUE_BELL_BEHAVIOUR_VIBRATE:
                BellHandler.getInstance(mActivity).doBell();
                break;
            case GoldBOXPropertyConstants.IVALUE_BELL_BEHAVIOUR_BEEP:
                loadBellSoundPool();
                if (mBellSoundPool != null)
                    mBellSoundPool.play(mBellSoundId, 1.f, 1.f, 1, 0, 1.f);
                break;
            case GoldBOXPropertyConstants.IVALUE_BELL_BEHAVIOUR_IGNORE:
    ...
    public void setTerminalShellPid(@NonNull TerminalSession terminalSession, int pid) {
        ...
        GoldBOXSession goldboxSession = service.getGoldBOXSessionForTerminalSession(terminalSession);
        if (goldboxSession != null)
    ...
    private synchronized void loadBellSoundPool() {
        if (mBellSoundPool == null) {
            ...
            try {
                mBellSoundId = mBellSoundPool.load(mActivity, com.goldbox.shared.R.raw.bell, 1);
            } catch (Exception e){
                // Catch java.lang.RuntimeException: Unable to resume activity {com.goldbox/com.goldbox.app.GoldBOXActivity}: android.content.res.Resources$NotFoundException: File res/raw/bell.ogg from drawable resource ID
    ...
    public void switchToSession(boolean forward) {
        ...
        int size = service.getGoldBOXSessionsSize();
        if (forward) {
        ...
        GoldBOXSession goldboxSession = service.getGoldBOXSession(index);
        if (goldboxSession != null)
            setCurrentSession(goldboxSession.getTerminalSession());
    }
    ...
    private void renameSession(TerminalSession sessionToRename, String text) {
        ...
        GoldBOXService service = mActivity.getGoldBOXService();
        if (service != null) {
            GoldBOXSession goldboxSession = service.getGoldBOXSessionForTerminalSession(sessionToRename);
            if (goldboxSession != null)
                goldboxSession.getExecutionCommand().shellName = text;
        }
    }
    ...
    public void addNewSession(boolean isFailSafe, String sessionName) {
        ...
        if (service.getGoldBOXSessionsSize() >= MAX_SESSIONS) {
            ...
            GoldBOXSession newGoldBOXSession = service.createGoldBOXSession(null, null, null, workingDirectory, isFailSafe, sessionName);
            if (newGoldBOXSession == null) return;

            TerminalSession newTerminalSession = newGoldBOXSession.getTerminalSession();
    ...
    public TerminalSession getCurrentStoredSessionOrLast() {
        ...
        if (stored != null) {
            ...
            GoldBOXSession goldboxSession = service.getLastGoldBOXSession();
            if (goldboxSession != null)
                return goldboxSession.getTerminalSession();
            else
                return null;
        }
    }
    ...
    public void removeFinishedSession(TerminalSession finishedSession) {
        ...
        int index = service.removeGoldBOXSession(finishedSession);
    ...
    public void goldboxSessionListNotifyUpdated() {
        mActivity.goldboxSessionListNotifyUpdated();
    }
    ...
    public void checkAndScrollToSession(TerminalSession session) {
        ...
        final ListView goldboxSessionsListView = mActivity.findViewById(R.id.terminal_sessions_list);
        if (goldboxSessionsListView == null) return;

        goldboxSessionsListView.setItemChecked(indexOfSession, true);
        // Delay is necessary otherwise sometimes scroll to newly added session does not happen
        goldboxSessionsListView.postDelayed(() -> goldboxSessionsListView.smoothScrollToPosition(indexOfSession), 1000);
    }
    ...
    public void checkForFontAndColors() {
        try {
            File colorsFile = GoldBOXConstants.GOLDBOX_COLOR_PROPERTIES_FILE;
            File fontFile = GoldBOXConstants.GOLDBOX_FONT_FILE;
